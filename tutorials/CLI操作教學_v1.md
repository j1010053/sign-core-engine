...
trait 內容與規則造成的間接傷害是**逐事件**算的：

```text
間接傷害 = 係數 × Σ max(event.before, event.after)
```

`before/after` 是那一筆事件在前後狀態各自波及的 sign 數。整個 leaf 的
`reach_before/reach_after` 聯集只回答「這批編輯總共碰到多少詞」，不會再拿來乘
事件總數。校準反例很直觀：9 筆各只碰 1 個詞的 local 事件約得 `0.9524`，9 筆
各碰全部 9 個詞的 global 事件約得 `0.5714`；兩者事件數相同，但波及面不同。
8／9 條 global 規則仍分別落在預設 `0.6` 閾值的兩側。
