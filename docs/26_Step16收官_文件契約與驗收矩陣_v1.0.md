...
> 仍以《架構修補11》為權威。P60/P64 亦已由 host persistence crate 落地；本檔只以
...
4. 多 parent 先做 3-way merge，再套主幹 changeset。`signs`／`traits`／
   `distribution` 逐項合併；`prosody`／`dsl_decls` 整塊比較；衝突即不建節點。
...
| 類別 | 現行行為 |
|---|---|
| Language 一級資料 | DSL declaration、prosody、distribution、trait／sign rename |
