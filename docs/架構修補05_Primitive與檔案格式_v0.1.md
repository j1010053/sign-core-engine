...
> **本修補新增**:P20–P28(見 §11)。與修補 01–04 出入處,以彙整檔 §1 + 本檔 §11 為準。
...
prosody         = μ σ Ft ω φ ι U          # 七層階層(P1)
...
| 組 | 節點 |
|---|---|
| **① 定義類**(`=`,無序) | `feature` / `symbol` / `class` / `prosody`(七層)/ 受控範疇引用 |
...
| **P22** | **條件語法擴展**:`/` 復用(環境=守衛,同一記法);**`else` 鏈 = Elsewhere Condition**(disjunctive 單趟、第一匹配勝出;與「兩條有序規則」的 feeding 語意明確區別;關閉 docs/02 §13 的 `Else:` 開放項);**tier 相對 adjacency**(模板可含 tier 引用,字串只是預設 tier);**Path 表達式**(`.` 欄位 / `[key]` slot / `~` 聯結);**三道界**:無量詞(需要時用引擎提供的具名謂詞)、無計算、無副作用必終止 |
