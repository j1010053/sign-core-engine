...
**裁決:不遷移。** 判準即 P20 邊界測試——dsl crate 認不認識這個語法?`Feature`/`Symbol`/`Class`/`Prosody`(音系原子與韻律域)由 DSL 獨立宣告；遷去 Language Definition 形會迫使獨立模式依賴 Language 語法，違反 P20 依賴方向。小寫 `prosody = …` 已廢棄；Language 的 `=` 只管 DSL 不認識的受控範疇、trait、global、sign。
