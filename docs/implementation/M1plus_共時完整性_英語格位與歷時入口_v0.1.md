...
> **狀態**：Step 13 source interface／Primitive Edit 已由
> `docs/verification/Step13_PrimitiveEdit與SourceIdentity_封板_v1.md` 再封板；Step 14
> Primitive-only ChangeSet 以 `docs/verification/Step14_ChangeSetInterpreter_封板_v1.md`
> 為現行封板契約。
...
`docs/verification/Step14_ChangeSetInterpreter_封板_v1.md`。v2 現為唯一 sidecar schema，
舊 v1 檔不直接 replay。
...
亦已於 2026-07-30 收官（見
`docs/verification/Step16_文件契約與驗收矩陣_v1.0.md`）。此清單本身仍只封板 M1+ 共時範圍，不代表
